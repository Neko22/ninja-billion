...
exports.echo = function(opts,params,cb) {
  ...
  opts.router_ip_address = params.router_ip_address;
  opts.interval = params.interval;
  this.save();

  payloadToSend.contents.push({ "type": "paragraph", "text": "Settings Updated"});
  /*
  if (payloadToSend.contents[1]) {
    payloadToSend.contents[1].text = "The Ninja CPU Monitor will update every " + params.poll_interval + " seconds.";
  } else {
    payloadToSend.contents.push({ "type": "paragraph", "text": "The Ninja CPU Monitor will update every " + params.poll_interval + " seconds."});
