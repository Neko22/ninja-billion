    ...
    process.nextTick(function() {
        setInterval(function() {
			if (opts.username != "" && opts.password != "") { 
				...
				var body = "";
				console.log("Network "+action+": Tick ["+opts.ip_address+":"+opts.port+opts.pages[opts.page]+"] ("+opts.username+"/"+opts.password+")");
				var request = http.get(options, function(res) {
					res.on("data", function(chunk) {
						console.log("Network "+action+": Received Chunk");
						body += chunk;
					});
					res.on('end', function () {
						clearTimeout(timeout);
						if (body == "" || body == undefined) {
							console.log("Network "+action+": No Data"); 
							cycle_page(action, opts);					
						} else {						
							...
							$("tr").each(function(tr_idx, tr_elem) {
								...
								$("td", tr_elem).each(function(td_idx, td_elem) {
									...
									if ($(td_elem).text()==action) {
										console.log("Network "+action+": Found Action");
										column = column_index;
									} else if (column_index == column) {
										var tmp = $(td_elem).text();
										console.log("Network "+action+": Found Possible Byte Count: "+tmp);
										if (!isNaN(tmp)) { 
											total=tmp;
						...
						console.log("Network "+action+": Error - "+e.message);
						cycle_page(action, opts);	
					});
				});
				var timeout = request.setTimeout(opts.interval, function( ) {
					console.log("Network "+action+": Timeout");
					res.abort();
					cycle_page(action, opts);				
				});
			} else {
				console.log("Network "+action+": No Username/Password");
			}
        }, (opts.interval*1000));
	}); 
};

function cycle_page(action, opts) {
