...
function ReceiveDevice(app, opts) {
    ...
    opts.received = "0"; // last total of bytes received, so we can calculate the k/s
        ...
        setInterval(function() {
			http.get({ host:opts.ip_address, port:80, path:'/getWanReceive.sh' }, function(res) {
				res.on("data", function(chunk) {
					...
					var per_second = 0;
					if (opts.received != 0) {
						per_second = Math.floor((total - opts.received) / interval); 
						console.log('Network Received: '+per_second+' k/s');
					}
					opts.received = total;
