	...
	*/
    ...
    process.nextTick(function() {
        setInterval(function() {
            get_network('getWanTransmit');
			get_network('getWanReceive');
        }, (opts.interval*1000));
	});
	...
	function get_network(type) {
		...
		http.get({ host:opts.router_ip_address, port:80, path:'/'+type+'.sh' }, function(res) {
			res.on("data", function(chunk) {
				var response = chunk.split("|");
				...
				var sent_per_second = Math.floor((sent_total - opts.sent) / interval); 
				console.log('Network Sent: '+sent_per_second+' k/s');
				opts.sent = sent_total;
				self.G = "billion"+type;
				self.V = 0; // 0 is Ninja Blocks' device list
				switch(type) {
					case "getWanTransmit": self.D = 540; break;
					case "getWanReceive": self.D = 530; break;
				}
